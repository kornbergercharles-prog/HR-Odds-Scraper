...
def parse_fanduel():
    try:
        with open("fanduel_responses.json", "r", encoding="utf-8") as f:
            raw = json.load(f)
        if not raw:
            print("FanDuel: no data")
            return pd.DataFrame(columns=["player", "fd_odds"])
        df = pd.DataFrame(raw)
        if "player" not in df.columns or "fd_odds" not in df.columns:
            print("FanDuel: missing columns")
            return pd.DataFrame(columns=["player", "fd_odds"])
        df = df.dropna(subset=["player", "fd_odds"])
        df = df.drop_duplicates(subset=["player"])
        print(f"FanDuel players: {len(df)}")
        return df
...
if len(dk_df) > 0 and len(fd_df) > 0:
    ...
    df = pd.DataFrame(columns=["player", "dk_odds", "fd_odds"])
...
out = pd.DataFrame({
    "Player": df["player"] if len(df) > 0 else [],
    "DK Odds": df["dk_odds"].fillna("—") if len(df) > 0 else [],
    "DK Impl. Prob": df["dk_implied"].apply(fmt_prob) if len(df) > 0 else [],
    "FanDuel Odds": df["fd_odds"].fillna("—") if len(df) > 0 else [],
    "FD Impl. Prob": df["fd_implied"].apply(fmt_prob) if len(df) > 0 else [],
    "Avg Impl. Prob": df["avg_implied"].apply(fmt_prob) if len(df) > 0 else [],
})
